...
| Dashboard 与截图 | 计划中（planned，P2 / Week 7 Portfolio） | 不属于 Phase E。 |
| Phase F Obsidian-ready 模式笔记 | 已实现（implemented） | repo-local export notes exist under [obsidian_pattern_notes/AI_Radar_Week2_MOC.md](../obsidian_pattern_notes/AI_Radar_Week2_MOC.md)；未导入任何 vault。 |
...
- Phase F: 仓库内 Obsidian-ready 模式笔记已实现；未来导入或提升为规则前仍需人工复核。
