...
AI Radar Agent 的目标不是自动生成一篇 AI 日报，而是把高噪声、高频变化、来源分散的
AI 行业信息，转化为可追溯、可审计、可发布的每日情报判断。它关注的不只是今天发生了什么，
更关注哪些信号值得相信、哪些事件值得进入 Top、哪些内容可以发布、哪些判断必须保留不确定性。

这个项目一开始就必须回答几个关键问题：消息源从哪里来，官方源、权威媒体和二手转述如何分层；
同一事件在多天内反复出现时如何历史去重；旧闻、跟进报道和真正的新信号如何区分；
证据如何绑定到报告里的判断；LLM 应该如何参与分析但不充当事实来源；发布到 Feishu 或 bot 前，
系统如何阻断低质量、无来源、重复或存在幻觉风险的内容。对这个 Agent 来说，真正的难点不是调用
LLM，而是设计一套稳定的证据、质量、去重和发布控制机制。

AI Radar 的实现方法是：先通过 RSS、Bocha 等公开来源召回信号，再通过时间窗口、消息源质量、
历史记录和 Evidence Gate 筛选证据；LLM 只在证据已经绑定的前提下做结构化综合、排序和表达；
报告生成后，还会经过 `report_lint`、`top_event_audit`、final-top reconciliation 和
Publish Gate，最后才进入私有生产环境中的 Feishu 文档和 bot 发布路径。也就是说，系统把召回、
证据、分析、审计、发布拆成多个可检查的阶段，而不是把整件事交给模型一次性完成。

这个脱敏公开镜像展示的是一个真实生产 Agent 如何被标准化并沉淀为作品集案例。它保留了 workflow、
策略面板、架构图、schema 契约、eval cases、脱敏 demo artifacts 和关键 runtime 逻辑；
同时排除了生产 secrets、真实 Feishu 发布历史、私有配置、生产 state 和 raw outputs。
它沉淀出的可复用能力包括：证据优先的情报工作流、消息源质量控制、历史去重、发布门禁、
Agent 自治边界、无外部副作用 eval、`RunManifest` / `ToolCall` 契约思维，以及生产项目如何转成安全公开镜像的方法论。
