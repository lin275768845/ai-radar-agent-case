...
先召回和过滤证据，再让 LLM 基于证据做结构化分析和报告生成。LLM 不被当作事实来源，
而是用于归纳、排序和表达。系统还通过 Evidence Gate、report_lint、top_event_audit、
发布门禁（Publish Gate）和人工控制边界，尽量避免低质量消息、重复事件、幻觉内容和误发布。
